...
df_agegroups['Age_Lower_Bound'] = df_agegroups['Age'].apply(
    extract_lower_bound)
...
df_chart['Sort'] = np.where(
    df_chart['Name'] == 'Other', df_chart['Count'].min() - 1, df_chart['Count'])
...
chart = alt.Chart(df_chart).mark_arc(innerRadius=60, outerRadius=120).encode(
    ...
    alt.Color('Name:N', sort=alt.EncodingSortField(field='Sort',
              order='descending'), legend=alt.Legend(labelLimit=0)),
    alt.Order('Sort:Q', sort='descending'),
    tooltip=[alt.Tooltip('Name:N', title='Type'), alt.Tooltip(
        'Count:Q', format=',', title='Count'), alt.Tooltip('Percentage:Q', format='.0%', title='Percentage')]
)
...
chart = alt.Chart(df_year).mark_area().encode(
    ...
    [alt.Tooltip('Year:O', title='Year'), alt.Tooltip(
...
df_year_gender = df_after_2003.groupby(
    ['Year', 'Gender'], as_index=False)['Count'].sum()
...
chart = alt.Chart(df_year_gender).mark_line().encode(
    ...
    alt.Color('Gender:N', legend=alt.Legend(
        orient='bottom', title=' ', padding=-20)),
    [alt.Tooltip('Year:N', title='Year'), alt.Tooltip(
        'Gender:N', title='Gender'), alt.Tooltip('Count:Q', format=',', title='Count')]
)
...
df_year_gender['Total'] = df_year_gender.groupby(
    'Year')['Count'].transform('sum')
df_year_gender['Percentage'] = df_year_gender['Count'] / \
    df_year_gender['Total']

chart = alt.Chart(df_year_gender).mark_area().encode(
    alt.X('Year:N', title=None, axis=alt.Axis(labelAngle=0)),
    alt.Y('Count:Q', stack='normalize', title=None),
    # Bug in Altair when using legend=None it cuts the y axis
    alt.Color('Gender:N', legend=alt.Legend(
        orient='bottom', title=' ', padding=-20)),
    [alt.Tooltip('Year:N', title='Year'), alt.Tooltip('Gender:N', title='Gender'), alt.Tooltip(
        'Count:Q', format=',', title='Count'), alt.Tooltip('Percentage:Q', format='.1%', title='Percentage')]
)
st.altair_chart(chart, use_container_width=True)
...
chart = alt.Chart(df_gender).mark_arc(innerRadius=60, outerRadius=120).encode(
    alt.Theta("Count:Q").stack(True),
    alt.Color("Gender:N"),
    tooltip=[alt.Tooltip('Gender:N', title='Type'), alt.Tooltip(
        'Count:Q', format=',', title='Count'), alt.Tooltip('Percentage:Q', format='.0%', title='Percentage')]
)
st.altair_chart(chart, use_container_width=True)
...
df_age['Age'] = pd.Categorical(
    df_age['Age'], categories=df_age['Age'].tolist(), ordered=True)
...
# Data by type
top_types = df_latest.groupby('Name')['Count'].sum(
).sort_values(ascending=False).head(20)
other_count = df_latest[~df_latest['Name'].isin(
    top_types.index)]['Count'].sum()
df_type = pd.concat([top_types.reset_index(), pd.DataFrame({'Name': ['Other'], 'Count': [
                    other_count]})], ignore_index=True).sort_values('Count', ascending=False)
df_type = pd.concat([df_type[df_type['Name'] != 'Other'],
                    df_type[df_type['Name'] == 'Other']])
